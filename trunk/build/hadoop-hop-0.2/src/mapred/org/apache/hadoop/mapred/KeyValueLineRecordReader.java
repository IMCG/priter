...
public class KeyValueLineRecordReader implements RecordReader<Text, Text> {
  ...
  //for iterative Mapreduce's combination sort
  public KeyValueLineRecordReader(Configuration job, InputStream in, long offset, long endOffset)
	throws IOException {
  
	  lineRecordReader = new LineRecordReader(in, offset, endOffset, job);
	  dummyKey = lineRecordReader.createKey();
	  innerValue = lineRecordReader.createValue();
	  String sepStr = job.get("key.value.separator.in.input.line", "\t");
	  this.separator = (byte) sepStr.charAt(0);
