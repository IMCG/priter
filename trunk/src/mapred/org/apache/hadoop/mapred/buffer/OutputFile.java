...
public class OutputFile implements Comparable<OutputFile>, Writable {
	...
	public abstract static class Header implements Writable, Comparable<Header> {
		...
		public static Header readHeader(DataInput in) throws IOException {
			...
			case PKVBUF: header = new PKVBufferHeader(); break;
			default: return null;
			}
	...
	public static class PKVBufferHeader extends Header {
		
		private int iteration;
		
		public PKVBufferHeader() { super(Type.PKVBUF, null, 0f, false); }
		
		public PKVBufferHeader(TaskAttemptID owner, int iteration) {
			super(Type.PKVBUF, owner, 0f, false);
		...
		public int compareTo(Header header) {
			if (header instanceof PKVBufferHeader) {
				PKVBufferHeader other = (PKVBufferHeader) header;
				if (owner.equals(other.owner)) {
		...
		@Override
		public boolean equals(Object o) {
			if (o instanceof PKVBufferHeader) {
				return this.compareTo((PKVBufferHeader) o) == 0;
			}
			return false;
		}
